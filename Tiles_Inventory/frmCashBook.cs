...
{
    public partial class frmCashBook : BaseForm
    {
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            List<CashBookReceive> lstCashBookReceive = new List<CashBookReceive>();
            List<CashBookPayment> lstCashBookPayment = new List<CashBookPayment>();

            lstCashBookReceive = new CashReceiveManager().GetAllDailyCashReceiveInformationByDate(fromDate, toDate, MTBFConstants.AppConstants.BranchID);
            lstCashBookPayment = new CashReceiveManager().GetAllDailyCashPaymentInformationByDate(fromDate, toDate, MTBFConstants.AppConstants.BranchID);

            decimal rTotal = 0;
            decimal eTotal = 0;

            DataTable Rdt = new DataTable();
            Rdt.Columns.Add("Receive Date");
            Rdt.Columns.Add("Line Item");
            Rdt.Columns.Add("Amount");


            foreach (CashBookReceive cashReceive in lstCashBookReceive)
            {
                DataRow row = Rdt.NewRow();
                row["Receive Date"] = cashReceive.ReceiveDate.ToString("yyyy/MM/dd");
                row["Line Item"] = cashReceive.Description;
                row["Amount"] = cashReceive.Amount;
                rTotal = rTotal + cashReceive.Amount;
                Rdt.Rows.Add(row);
            }
            grvCashReceveInfo.DataSource = Rdt;

            DataTable Edt = new DataTable();
            Edt.Columns.Add("Expense Date");
            Edt.Columns.Add("Line Item");
            Edt.Columns.Add("Amount");
            foreach (CashBookPayment cPayment in lstCashBookPayment)
            {

                DataRow row = Edt.NewRow();
                row["Expense Date"] = cPayment.ExpenseDate.ToString("dd/MM/yyyy");
                row["Line Item"] = cPayment.Description;
                row["Amount"] = cPayment.Amount;
                eTotal = eTotal + cPayment.Amount;
                Edt.Rows.Add(row);
            }

            grvExpenseInformation.DataSource = Edt;

            //LoadCashReceiveInformation(fromDate, toDate);
            //LoadOtherReceiveInformation(fromDate, toDate);
            ...
            //LoadSalesReturnInformation(fromDate, toDate);
            ...
            txtOpeningBalance.Text = new CashReceiveManager().GetPreviousBalanceByDate(fromDate, MTBFConstants.AppConstants.BranchID).ToString("N", nfi);
            txtTotalIncome.Text = rTotal.ToString("N", nfi);
            txtTotalExpense.Text = eTotal.ToString("N", nfi);
            txtReceiveTotal.Text = rTotal.ToString("N", nfi);
            txtExpenseTotal.Text = eTotal.ToString("N", nfi);
        ...
        private void LoadSalesReturnInformation(string fromDate, string toDate)
        {
            List<SalesReturn> lstSalesReturn = new List<SalesReturn>();
            string filter = string.Format("{0} between '{1}' and '{2}' And BranchID={3} and OrganizationID={4} ", "ReturnDate", fromDate, toDate, MTBFConstants.AppConstants.BranchID, MTBFConstants.AppConstants.OrganizationID);
            lstSalesReturn = new SalesReturnManager().GetFilteredSalesReturn(filter).Cast<SalesReturn>().ToList();

            if (lstSalesReturn.Count > 0)
            {
                foreach (SalesReturn sReturn in lstSalesReturn)
                {
                    UltraGridRow row = grvExpenseInformation.DisplayLayout.Bands[0].AddNew();
                    row.Cells["Expense Date"].Value = sReturn.ReturnDate.ToString("dd/MM/yyyy");
                    row.Cells["Line Item"].Value = "Sales Return";
                    row.Cells["Amount"].Value = sReturn.PaidAmount;
        ...
        private void LoadOtherReceiveInformation(string fromDate, string toDate)
        {
            string filter = string.Format("{0} between '{1}' and '{2}'", "ReceiveDate", fromDate, toDate);

            List<OtherReceive> lstCashReceive = new CashReceiveManager().GetFilteredOtherReceive(filter).Cast<OtherReceive>().ToList();
            foreach (OtherReceive cashReceive in lstCashReceive)
            {
                UltraGridRow row = grvCashReceveInfo.DisplayLayout.Bands[0].AddNew();
                row.Cells["Receive Date"].Value = cashReceive.ReceiveDate.ToString("yyyy/MM/dd");
                row.Cells["Line Item"].Value = "Other Receive";
                row.Cells["Amount"].Value = cashReceive.ReceiveAmount;
            }
            CalculateReceiveAmount();
        }
        ...
        private void CalcualateOpeningBalance()
        {
            ...
            DateTime openingBalanceDate = Convert.ToDateTime(dtpFromDate.Value.ToString("yyyy/MM/dd") + MTBFConstants.DAY_START_TIME);

            List<CashReceive> lstCashReceive = new CashReceiveManager().GetFilteredCashReceive(filter);
            lstCashReceive = lstCashReceive.Cast<CashReceive>().Where(c => c.ReceiveDate < openingBalanceDate).ToList();


            List<OtherReceive> lstOtherReceive = new CashReceiveManager().GetFilteredOtherReceive(filter);
            lstOtherReceive = lstOtherReceive.Cast<OtherReceive>().Where(c => c.ReceiveDate < openingBalanceDate).ToList();


            List<BankWithdraw> lstBankWithdraw = new BankAccountManager().GetFilteredBankWithdraw(filter);
            lstBankWithdraw = lstBankWithdraw.Cast<BankWithdraw>().Where(c => c.WithdrawDate < openingBalanceDate).ToList();

            List<SalesOrder> lstSalesOrder = new SalesManager().GetAllSalesOrderByBranchAndOrganization(MTBFConstants.AppConstants.BranchID, MTBFConstants.AppConstants.OrganizationID);
            lstSalesOrder = lstSalesOrder.Cast<SalesOrder>().Where(c => c.SalesDate < openingBalanceDate).ToList();


            List<Expense> lstExpense = new ExpenseManager().GetFilteredExpense(filter);
            lstExpense = lstExpense.Cast<Expense>().Where(c => c.ExpenseDate < openingBalanceDate).ToList();

            List<BankDeposit> lstBankDeposit = new List<BankDeposit>();
            lstBankDeposit = new BankAccountManager().GetFilteredBankDeposit(filter);
            lstBankDeposit = lstBankDeposit.Cast<BankDeposit>().Where(c => c.DepositDate < openingBalanceDate).ToList();


            List<Payment> lstPayment = new List<Payment>();
            lstPayment = new PaymentManager().GetFilteredPaymentInformation(filter).Cast<Payment>().ToList();
            lstPayment = lstPayment.Cast<Payment>().Where(c => c.PaymentDate < openingBalanceDate).ToList();

            List<SalesReturn> lstSalesReturn = new List<SalesReturn>();
            lstSalesReturn = new SalesReturnManager().GetFilteredSalesReturn(filter).Cast<SalesReturn>().ToList();
            lstSalesReturn = lstSalesReturn.Cast<SalesReturn>().Where(c => c.ReturnDate < openingBalanceDate).ToList();
            ...
            foreach (OtherReceive otherReceive in lstOtherReceive)
            {
                receiveAmount = receiveAmount + otherReceive.ReceiveAmount;
            }
            ...
            foreach (SalesReturn salesReturn in lstSalesReturn)
            {
                expenseAmount = expenseAmount + salesReturn.PaidAmount;
            }
            ...
            decimal balance = receiveAmount - expenseAmount;
        ...
        private DataTable BuildCashBookTable(List<CashBook> lstCashBook)
        {
            ...
            foreach (CashBook cBook in lstCashBook)
            {
                ...
                {
                    row = table.NewRow();
                    row["Date"] = dtpFromDate.Value.AddDays(-1).ToString("dd/MM/yyyy");
                    row["Description"] = "Opening Balance";
                    row["Received"] = Convert.ToDecimal(txtOpeningBalance.Text);
                    row["Balance"] = Convert.ToDecimal(txtOpeningBalance.Text);
                    table.Rows.Add(row);
                    count++;
                }
